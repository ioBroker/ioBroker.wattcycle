...
export class WattCycleBle {
     ...
     */
    private async findPeripherals(targetMacs: string[], timeoutMs: number): Promise<Map<string, any>> {
        ...
        return new Promise((resolve, reject) => {
            ...
            const finish = (stopScan: boolean): void => {
                if (settled) {
                    return;
                }
                settled = true;
            ...
            this.noble.on('discover', onDiscover);
            this.noble.startScanningAsync([], false).catch((e: Error) => fail(e));
        });
    }
     ...
     */
    public async readBatteries(macs: string[]): Promise<Map<string, BatteryReadResult | Error>> {
        if (this.busy) {
            throw new Error('BLE adapter is busy');
        }
        this.busy = true;
        const results = new Map<string, BatteryReadResult | Error>();
        try {
            await this.waitPoweredOn();
            ...
            for (const mac of macs) {
                ...
                try {
                    const data = await withTimeout(
                        this.readPeripheral(peripheral, mac),
    ...
    private async readPeripheral(peripheral: any, mac: string): Promise<BatteryReadResult> {
